...
class BGUploaderListenerModule(val reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext), LifecycleEventListener {
  private val TAG = "BGUploader"
  private var notificationChannelID = "EBM_NOTIFICATION_CH"
  ...
  override fun getName(): String {
    return "BGUploaderListenerModule"
  }
  ...
  fun startUpload(options: ReadableMap, promise: Promise) {
    ...
    if(!isGlobalRequestObserver) {
      ...
      GlobalRequestObserver(application, BGUploaderGRObserverDelegate(reactContext))
    }
  ...
  private fun createNotificationChannel() {
    ...
    if (Build.VERSION.SDK_INT >= 26) {
      ...
      if (manager != null && manager.getNotificationChannel(notificationChannelID) == null) {
          val channel = NotificationChannel(notificationChannelID, CHANNEL_NAME, NotificationManager.IMPORTANCE_NONE)
